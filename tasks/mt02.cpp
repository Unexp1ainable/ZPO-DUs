...
/*	TASK 2.1 - Geometry transformation
        ...
        The function performs a geometric transformation of the image using
   nearest neighbor interpolation.

        Input Transform describes the transformation of pixels from the input
   image to the output image. We use the inverse procedure (see lecture), so we
   calculate the inverse transformation and for each point of the output image
   we will look for its position in the input image and use nearest neighbor
   interpolation to find the value of the output pixel.

        Input (src) image is in grayscale (single channel 8bit).
*/

int geometricalTransform(const cv::Mat &src, cv::Mat &dst,
  ...
  /*
          ...
          1. find its position in the source image (using the prepared inverse
     transformation in the matrix T) see
     http://docs.opencv.org/modules/imgproc/doc/geometric_transformations.html?highlight=warpaffine#warpaffine
          ...
          3. use the nearest neighbor interpolation to calculate the resulting
     brightness of the target pixel (use the cvRound () function)

          Allowed Mat attributes, methods and OpenCV functions for task solution
     are: Mat:: rows, cols, step(), size(), at<>(), zeros(), ones(), eye(),
     cvRound()
  */

  /* ***** Working area - begin ***** */
  cv::Mat srcPoint = cv::Mat::ones(cv::Size(1, 3), CV_32FC1);
  cv::Mat dstPoint = cv::Mat::ones(cv::Size(1, 3), CV_32FC1);
  for (int y = 0; y < dst.rows; y++) {
    for (int x = 0; x < dst.cols; x++) {
      dstPoint.at<float>(0, 0) = y;
      dstPoint.at<float>(0, 1) = x;

      // transform
      srcPoint = T * dstPoint;

      // check border
      float srcx = cvRound(srcPoint.at<float>(1, 0));
      float srcy = cvRound(srcPoint.at<float>(0, 0));
...
//---------------------------------------------------------------------------
void checkDifferences(const cv::Mat test, const cv::Mat ref, std::string tag,
                      bool save = false);
//---------------------------------------------------------------------------
...
int main(int argc, char *argv[]) {
  ...
  /*
          ...
          - we want to rotate counterclockwise (the rotation matrix must be
     inverted before use),
          - multiplication of matrices A*B*C is in c++ from the left, it is
     necessary to use parentheses (A*(B*C)), if we want to multiply from the
     right

          Allowed Mat attributes, methods and OpenCV/math functions for task
     solution are: Mat:: rows, cols, at<>(), zeros(), ones(), eye(), inv(), cos,
     sin, CV_PI
  */
  ...
  float rot = r * (CV_PI / 180);
  R.at<float>(0, 0) = std::cos(rot);
  R.at<float>(1, 0) = std::sin(rot);
  R.at<float>(0, 1) = -std::sin(rot);
  R.at<float>(1, 1) = std::cos(rot);
  ...
  // compute reference solution - the warpAffine function internally inverts the
  // transformation matrix and interpolates the output pixel values from the
  // input image
  cv::Mat Mref = cv::getRotationMatrix2D(c, r, s);
  cv::warpAffine(src_gray, tran_ref, Mref, src_gray.size(), cv::INTER_NEAREST,
